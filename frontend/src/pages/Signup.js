...
function Signup() {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      case 'name':
        ...
        else if (!/^[A-Za-z]+(\s[A-Za-z]+)*$/.test(value)) error = 'Only letters and spaces allowed';
        break;
      case 'mobile':
      ...
      case 'username':
        ...
        else if (!/^[A-Za-z][A-Za-z0-9]*$/.test(value)) error = 'Username must start with a letter';
        else {
          const users = JSON.parse(localStorage.getItem('users')) || [];
          const existing = users.find(u => u.username === value);
          if (existing) error = 'Username already exists';
        }
        break;
      case 'password':
        if (!value.trim()) error = 'Password is required';
        else if (!/^(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{6,}$/.test(value)) error = 'Password must include uppercase, digit & special char';
        break;
      ...
      case 'policeId':
        if (form.role === 'Police') {
          ...
          else if (!allowedPoliceIDs.includes(value)) error = 'Invalid Police ID';
        }
        break;
      case 'image':
  ...
  const handleSubmit = (e) => {
    ...
    if (form.role !== 'Police') delete userData.policeId;
    ...
    <div className="rainbow-bg">
      <div className="signup-box">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          {form.role === 'Police' && (
            <>
              <input name="policeId" placeholder="Police ID" value={form.policeId} onChange={handleChange} />
...
/>
          ...
          {form.image && <img src={form.image} alt="Preview" style={{ width: 100, borderRadius: '50%' }} />}
